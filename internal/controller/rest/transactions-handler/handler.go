...
func New(transactionsHandler transactionsHandler) *Handler {
	return &Handler{
		transactionsHandler: transactionsHandler,
	}
}
...
func (h *Handler) Deposit(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.transactionsHandler.Deposit(ctx, transaction, userInfo.UserID); err != nil {
		switch {
...
func (h *Handler) Withdraw(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.transactionsHandler.Withdraw(ctx, transaction, userInfo.UserID); err != nil {
		switch {
		case errors.Is(err, entity.ErrWalletNotFound):
			http.Error(w, "wallet not found", http.StatusNotFound)
...
func (h *Handler) Transfer(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.transactionsHandler.Transfer(ctx, transaction, userInfo.UserID); err != nil {
		switch {
		case errors.Is(err, entity.ErrWalletNotFound):
			http.Error(w, "wallet not found", http.StatusNotFound)
...
func (h *Handler) GetTransactions(w http.ResponseWriter, r *http.Request) {
	...
	transactions, err := h.transactionsHandler.GetTransactions(ctx, request, entity.WalletID(walletID), userInfo.UserID)
	if err != nil {
