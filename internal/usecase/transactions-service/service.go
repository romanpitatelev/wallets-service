...
func New(
	...
	return &Service{
		walletsStore:      walletsStore,
		transactionsStore: tratransactionsStore,
		xrClient:          xrClient,
		tx:                tx,
		producer:          producer,
		metrics:           newMetrics(),
...
func (s *Service) Deposit(ctx context.Context, transaction entity.Transaction, userID entity.UserID) error {
	...
	if err := s.tx.DoWithTx(ctx, func(ctx context.Context) error {
		dbWallet, err := s.walletsStore.GetWallet(ctx, *transaction.ToWalletID, userID)
		if err != nil {
...
func (s *Service) Withdraw(ctx context.Context, transaction entity.Transaction, userID entity.UserID) error {
	...
	if err := s.tx.DoWithTx(ctx, func(ctx context.Context) error {
		dbWallet, err := s.walletsStore.GetWallet(ctx, *transaction.FromWalletID, userID)
		if err != nil {
			return fmt.Errorf("wallet not found: %w", err)
		}
...
func (s *Service) Transfer(ctx context.Context, transaction entity.Transaction, userID entity.UserID) error {
	...
	if err := s.tx.DoWithTx(ctx, func(ctx context.Context) error {
		dbFromTransferWallet, err := s.walletsStore.GetWallet(ctx, *transaction.FromWalletID, userID)
		if err != nil {
			return fmt.Errorf("wallet not found: %w", err)
		}

		dbToTransferWallet, err := s.walletsStore.GetWallet(ctx, *transaction.ToWalletID, userID)
		if err != nil {
			return fmt.Errorf("wallet not found: %w", err)
		}
...
//nolint:lll
func (s *Service) GetTransactions(ctx context.Context, request entity.GetWalletsRequest, walletID entity.WalletID, userID entity.UserID) ([]entity.Transaction, error) {
